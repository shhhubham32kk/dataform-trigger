...
def trigger_pipeline():
    try:
        ...
        if not blob.exists():
            return jsonify({"status": "skipped", "message": "raw/sales.csv not found"}), 404
        ...
        job_config = bigquery.LoadJobConfig(
            ...
            skip_leading_rows=1,
            write_disposition=bigquery.WriteDisposition.WRITE_TRUNCATE, # Cleans staging out first
            autodetect=False # Uses your exact schema layout
        )
        ...
        url = f"https://dataform.googleapis.com/v1beta1/projects/{PROJECT_ID}/locations/{LOCATION}/repositories/{REPO_ID}/workflowInvocations"
        ...
        body = {
            "workflowConfig": f"projects/{PROJECT_ID}/locations/{LOCATION}/repositories/{REPO_ID}/workflowConfigs/{WORKFLOW_CONFIG_ID}"
        }
        ...
        if response.status_code in [200, 201]:
            ...
            return jsonify({"status": "success", "dataform_run": response.json()}), 200
        else:
